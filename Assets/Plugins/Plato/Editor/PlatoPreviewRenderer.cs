...
namespace Plato
{
    public class PlatoPreviewRenderer
    {
        ...
        public void Render(Material material, Rect rect, GUIStyle background)
        {
            ...
            _utility.camera.transform.position = rotation * Vector3.forward * -3;
            _utility.camera.transform.LookAt(Vector3.zero);

            _utility.lights[0].intensity = 1;
            _utility.lights[0].transform.rotation = Quaternion.Euler(50, 50, 0);
            _utility.lights[1].intensity = 1;
